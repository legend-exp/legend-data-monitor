...
def get_periods(key: str, datasets_path: Path) -> dict[str, list[tuple[str, str]]]:
    runlists_path = datasets_path / "runlists.yaml"
...
def get_geds(
    ...
    runinfo_path = os.path.join(datasets_path, "runinfo.yaml")
    with open(runinfo_path) as f:
        runinfo = yaml.load(f, Loader=yaml.CLoader)
    runs_key = runlists[key]
...
def get_strings_info(auto_dir_path: str, period: str) -> dict:
    ...
    lmeta = LegendMetadata(lmeta_path)

    runinfo_path = os.path.join(lmeta_path, "datasets/runinfo.yaml")
    with open(runinfo_path) as f:
        runinfo = yaml.load(f, Loader=yaml.CLoader)

    # period missing from runinfo
    available_periods = sorted(runinfo.keys())
    period_before = available_periods[-1]
    found = False
    prev = None
    ...
    if not found:
        utils.logger.warning(
            f"{period} not found in runinfo, using fallback {period_before}"
        )

    first_run = sorted(runinfo[period_before].keys())[0]
    data_key = sorted(runinfo[period_before][first_run].keys())[0]
    start_key = runinfo[period_before][first_run][data_key]["start_key"]
...
def generate_dashboard(
    ...
    add_shift_log_sheet(
        strings_info, output_path, period, current_run, threshold_entries
    )  # log summary
