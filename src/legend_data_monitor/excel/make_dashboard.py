...
def generate_period_colors(period_list: list[str]) -> dict[str, str]:
    ...
    Preferred colors are used for known periods. Unknown periods get
    auto-generated colors distributed evenly around the color wheel.
    """
    colors = {}
    unknown_periods = [p for p in period_list if p not in PREFERRED_PERIOD_COLOURS]

    # preferred colors for known periods
    for period in period_list:
        if period in PREFERRED_PERIOD_COLOURS:
            colors[period] = PREFERRED_PERIOD_COLOURS[period]

    # generate colors for unknown periods
    if unknown_periods:
        n = len(unknown_periods)
        for i, period in enumerate(unknown_periods):
            hue = (i / n * 0.85) + 0.1
            saturation = 0.7
            lightness = 0.5
            # convert HSL to RGB
...
def make_excel(
    ...
    period_colors = generate_period_colors(period_list)
    ...
    for period in period_list:
        ...
        color = period_colors.get(period, DEFAULT_PERIOD_COLOUR)
...
QCP_CAL_CHECKS = [
    ("FEP_gain_stab", "FEP_gain_stab", "F6D7B8"),
    ("fwhm_ok", "fwhm_ok", "F6D7B8"),
    ("npeak", "npeak", "F6D7B8"),
    ("const_stab", "const_stab", "F6D7B8"),
]
QCP_PHY_CHECKS = [
    ("baseln_spike", "baseln_spike", "FFD0E8"),
    ("baseln_stab", "baseln_stab", "FFD0E8"),
    ("pulser_stab", "pulser_stab", "FFD0E8"),
    ("saturated_rate", "saturated_rate", "FFD0E8"),
    ("discharge_rate", "discharge_rate", "FFD0E8"),
    ("tot_discharge_dead_time", "tot_discharge_dead_time", "FFD0E8"),
]
QCP_ESCALE_CHECKS = [
    ("escale_fwhm_FEP", "escale_fwhm_FEP", "D9E1F2"),
    ("escale_fwhm_583", "escale_fwhm_583", "D9E1F2"),
    ("escale_FEP_pos", "escale_FEP_pos", "D9E1F2"),
    ("escale_SEP_residual", "escale_SEP_residual", "D9E1F2"),
]
QCP_PSD_CHECKS = [
    ("AoE_stab", "AoE_stab", "95B3DE"),
]
