...
def box_summary_plot(
    ...
    for ged, item in results.items():
        ...
        if pars:
            entry = det_info['detectors'][ged]["channel_str"]
            ged_par = pars[ged] if ged in pars.keys() else pars[entry]
            fwhm = (
                (((ged_par or {}).get("results", {}))
...
def get_calib_data_dict(
    ...
    validity_file = os.path.join(pars[2 if tier == "hit" else 3], "validity")
...
def filter_series_by_ignore_keys(
    ...
    series_to_filter = series_to_filter.copy()

    # ensure datetime index safely
    if not isinstance(series_to_filter.index, pd.DatetimeIndex):
        series_to_filter.index = pd.to_datetime(
            series_to_filter.index, errors="coerce", utc=True
        )

    if series_to_filter.index.isna().any():
        raise ValueError("Index contains non-parsable timestamps after conversion.")
